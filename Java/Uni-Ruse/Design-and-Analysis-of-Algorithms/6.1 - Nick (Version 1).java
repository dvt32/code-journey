...
// To optimize
// - remove currentPosition, rather use i+1 to get the currentPosition
// - use StringBuilder instead of String - saves some resources
			...
			switch (currentPosition % 2) {
				case 0:
					if (currentCharIsLetter) {
						output += Character.toString(Character.toLowerCase(currentChar));
					}
					else {
						output += Character.toString(currentChar);
					}
					break;
				case 1: 
					if (currentCharIsLetter) {
						output += Character.toString(Character.toUpperCase(currentChar));
					}
					else {
						output += Character.toString(currentChar);
					}
