...
27. Зададен е двумерен масив А с m реда n стълба, съдържащ ЕГН-та на група лица. Да се състави програма, която намира 
най-младата жена във всеки ред на двумерниямасив.
...
void main(){
	...
	char A[30][30][11], A_youngestWoman[30][11], // A_youngestWoman - ЕГН на най-младата жена на всеки ред
	...
	for (i = 0; i < m; i++){
		...
		for (j = 0; j < n; j++){
			...
			if (godina[i][j] > godina_max && gender[i][j] % 2 == 1)
			{
				...
				strcpy_s(A_youngestWoman[i], A[i][j]);
	...
	for (i = 0; i < m; i++){
		if (strcmp(A_youngestWoman[i], "\0") == 0) { cout << "На " << i << " ред няма ЕГН-та на жени." << endl; }
		else cout << "ЕГН-то на най-младата жена на " << i << " ред е " << A_youngestWoman[i] << "." << endl;
	}
